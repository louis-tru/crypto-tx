...
export default class BN {
	...
	private _init (number, base, endian) {
	...
	private _initNumber (number, base, endian) {
		if (number < 0) {
			this.negative = 1;
			number = -number;
		}
	...
	private _initArray (number, base, endian) {
	...
	private _parseHex (number, start) {
	...
	private _parseBase (number, base, start) {
		// Initialize as zero
		this.words = [ 0 ];
		this.length = 1;
	...
	private _countBits = Math.clz32 ? function _countBits (w) {
	...
	private _ishlnsubmul (num, mul, shift) {
